...
const MapRoute: React.FC<MapRouteProps> = ({ route, color, media }) => {
  // Convert path to LatLngExpression with fallback to prevent errors
  const path: LatLngExpression[] = route.path ? route.path.map((coord) => [coord[0], coord[1]]) : [];
  const routeColor = color || 'blue'; // Default color

  return (
    <Polyline positions={path} color={routeColor} weight={5}>
      <Popup>
        <div>
